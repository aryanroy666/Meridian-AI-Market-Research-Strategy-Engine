...
If any stage comes back with an empty result — no research tasks, no sources found, no evidence extracted — even after its retries, the pipeline stops immediately and marks the job `failed`, rather than letting a thin, low-confidence pipeline run limp forward into a report that reads as complete but isn't.
...
| **Citation coverage as a metric** | The system either links a finding to a citation or (implicitly) doesn't — there's no reported percentage of findings that ended up fully cited. |
