...
> Interactive, always-current documentation is also generated automatically by FastAPI at `/docs` (Swagger UI) and `/redoc` on any running instance of the backend. This file is the narrative companion to that — what each route is for and how the pieces fit together — not a replacement for it.
...
Submits a new research brief. This call runs the **entire seven-stage pipeline synchronously** — Planning → Research → Extraction → Validation → Citations → Report → Linking — and only returns once it finishes. There is no separate "poll for status" route; the frontend's animated progress screen is a client-side timeline rather than a server-pushed status feed (see [Known Limitations](../README.md#known-limitations) in the main README).
...
| `GET` | `/api/research/{job_id}/report` | The final linked report — this is what backs the Report / Evidence / Sources tabs in the UI |
...
| `500 Internal Server Error` | Unhandled backend error — mapped centrally in `backend/core/errors.py` |
